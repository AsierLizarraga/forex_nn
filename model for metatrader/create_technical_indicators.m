...
%     stosc = stochosc(ts, 25, 3, 'e','HighName','ALTO','LowName','BAJO','CloseName','CIERRE');
%     dataset = [dataset fts2mat(stosc.SOK)];
%     dataset = [dataset fts2mat(stosc.SOD)];
%     names{indice} = '%K_stockastic_25';
%     indice = indice  + 1;
%     names{indice} = '%D_stockastic_K_25_D_3';
%     indice = indice  + 1;
...
%     momts = tsmom(ts,25);
%     dataset = [dataset fts2mat(momts.CIERRE)];
%     names{indice} = 'Momentum_cierre_25';
%     indice = indice  + 1;
...
if inc_volroc == 1
    ...
    indice = indice  + 1;
%     
%     vrocts = volroc(ts,15,'VolumeName','VOLUMEN');
%     dataset = [dataset fts2mat(vrocts.VolumeROC)];
%     names{indice} = 'vrc_15';
%     indice = indice  + 1;
%    
%     vrocts = volroc(ts,50,'VolumeName','VOLUMEN');
%     dataset = [dataset fts2mat(vrocts.VolumeROC)];
%     names{indice} = 'vrc_50';
%     indice = indice  + 1;
%     
...
if inc_rsindex == 1
   ...
   indice = indice  + 1;
%    
%    rsits = rsindex(ts,50,'CloseName','CIERRE');
%    dataset = [dataset fts2mat(rsits.RSI)];
%    names{indice} = 'RSI_50';
%    indice = indice  + 1;
...
%    dataset = [dataset fts2mat(midfts.CIERRE)];
%    names{indice} = 'MID_BOLINGER_10';
%    indice = indice  + 1;


%    [midfts, upprfts, lowrfts] =  bollinger(ts,20,0,2);
%    dataset = [dataset fts2mat(upprfts.CIERRE)];
%    names{indice} = 'UP_BOLINGER_20';
%    indice = indice  + 1;
%    dataset = [dataset fts2mat(lowrfts.CIERRE)];
%    names{indice} = 'LOW_BOLINGER_20';
%    indice = indice  + 1;
  

end

% filter the 100 first observation = has nan: 51

classdef fitnes
    ...
    methods
        ...
        function obj =  fitnes(pred,dataset,ts,initial_deposit,symbol,lot_value,min_lot,pip,leverage,fitness_function,spread,comision_vol,tendencia,min_orders,mode,max_time)

   
            % call estrategy function: create a book the orders
            [ordenes,acu_profits,buy_signals,sell_signals] = estrategia1(pred,ts,dataset,initial_deposit,symbol,lot_value,min_lot,pip,leverage,spread,comision_vol,mode,tendencia,max_time);
            % obtain the orders
            obj.orders = ordenes;
            ...
            [profit_and_loss,average_return,sharpe_ratio,num_orders,stirlling_ratio,profit_loss_sharpe_ratio,profit_loss_stirlling_ratio,average_profit,profit_loss_sharpe_ratio_x_prof_rate,profit_loss_stirlling__x_prof_rate,profit_and_loss_x_prof_rate,profit_rate] = financial_measures(ordenes,initial_deposit,min_orders);
...
if n == 1 && orders(1).id == -1
    ...
    if n <= min_orders
        ...
        sharpe_ratio = -99999999;
        stirlling_ratio = -99999999;
        num_orders = n;
        profit_and_loss = -99999999;
        average_return = -1;
        profit_loss_sharpe_ratio = -99999999;
        profit_loss_stirlling_ratio = -99999999;
        average_profit = -99999999;
        profit_rate = -99999999;
        profit_loss_sharpe_ratio_x_prof_rate = -99999999;
        profit_loss_stirlling__x_prof_rate = -99999999;
        profit_and_loss__x_prof_rate = -99999999;
...
function [orders,acu_profits,buy_signals,sell_signals] = estrategia1(pred,ts,dataset,initial_deposit,symbol,lot_value,min_lot,pip,leverage,spread,comision_vol,mode,tendencia,max_time)
...
% create de order
or = order(-1,datestr(dataset(2,1)),'buy',1,symbol,dataset(2,2),-1,'entry buy low');
orders(1) = or;
...
for i=2:n-1
    ...
    % entry buy 
    if () && entry_buy == 0 && (hora >= 7 && hora <= 17) && (mode == 1) && (tend == tendencia || tendencia == 0)
        ...
        if aux_vol_buy > vol_sell
            ...
            if aux_free_margin > 0;
                ...
                % create de order
                or = order(id,datestr(dataset(i+1,1)),'buy',1,symbol,dataset(i+1,2),-1,'entry buy');
            ...
            % create de order
             or = order(id,datestr(dataset(i+1,1)),'buy',1,symbol,dataset(i+1,2),-1,'entry buy high');
    ...
    % entry sell 
    if && (mode == 2) && (tend == tendencia || tendencia == 0)
        ...
        if aux_vol_sell  > vol_buy
            ...
            if aux_free_margin > 0;
                ...
                % create de order
                or = order(id,datestr(dataset(i+1,1)),'sell',1,symbol,dataset(i+1,2),-1,'entry sell');
    ...
    % exit buy
    if   && entry_buy == 1) || (entry_buy == 1 && i == n - 1) 
...
%exit a buy order if overpass the time parameter
% 
%     if(entry_buy == 1)
%        
%         or = buy_order;
%         
%         if(or.dates >= or.dates + max_time)
%            
%         entry_buy = 0;
%         
%         % put the tiem and tehe exit price
...
%     end
%     
% % exit sell order if overpass the maximun time
%   
%    if(entry_sell == 1)
%        
%         or = sell_order;
